    ...
    >
      <div className={getInnerClass(collapsed)}>
        <div className={getHeaderRowClass(collapsed)}>
          {!collapsed && <h2 className={sidebarStyles.menuTitle}>Menu</h2>}
          <button
            type="button"
            onClick={toggle}
        ...
        <nav className={sidebarStyles.nav} role="navigation" aria-label="Main navigation">
          {options.map((option) => {
      ...
      <footer className={getFooterClass(collapsed)}>
        {!collapsed && (
          <p className={sidebarStyles.version}>Version {packageInfo.version}</p>
        )}
      </footer>
    </aside>
  )
